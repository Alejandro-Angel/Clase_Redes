...
// Ajuste fino SOLO para el regreso desde AZUL.
// Viene desde +900 hacia 0.
// Como se estaba pasando, este valor positivo lo deja un poco antes,
// todavia del lado azul.
const long POS_CENTRO_DESDE_AZUL = 20;
...
const int ROJO_R_MIN = 85;
const int ROJO_R_MAX = 105;

const int ROJO_G_MIN = 160;
const int ROJO_G_MAX = 180;

const int ROJO_B_MIN = 140;
const int ROJO_B_MAX = 160;

const int VERDE_R_MIN = 140;
const int VERDE_R_MAX = 165;

const int VERDE_G_MIN = 125;
const int VERDE_G_MAX = 140;

const int VERDE_B_MIN = 125;
const int VERDE_B_MAX = 140;

const int AZUL_R_MIN = 135;
const int AZUL_R_MAX = 155;

const int AZUL_G_MIN = 135;
const int AZUL_G_MAX = 155;

const int AZUL_B_MIN = 90;
const int AZUL_B_MAX = 120;

const int NEGRO_R_MIN = 160;
const int NEGRO_R_MAX = 190;

const int NEGRO_G_MIN = 180;
const int NEGRO_G_MAX = 200;
...
bool emergenciaSerialActiva = false;
...
void setup() {
  ...
  Serial.println("VERDE -> 0    -> SERVO SIN MOVER MOTOR");
  Serial.println("AZUL  -> 900  -> SERVO -> CENTRO AJUSTADO");
...
void loop() {
  ...
  if (!sistemaHabilitado || emergenciaSerialActiva) {
    detenerMotor();
    return;
  }
...
void gestionarEsperaMovimiento() {
  ...
  if (
    colorPendiente == COLOR_VERDE &&
    destinoPendiente == POS_VERDE
  ) {

    Serial.println("Pieza VERDE detectada.");
    Serial.println("Ya esta en la posicion central.");
    Serial.println("No se mueve el motor. Activando servo.");
...
void finalizarDestino() {
  ...
  if (!regresandoAlCentro) {
    ...
    if (
      ...
      long centroRetorno = POS_CENTRO;

      if (colorPendiente == COLOR_AZUL) {
        centroRetorno = POS_CENTRO_DESDE_AZUL;
        Serial.println("Ajuste aplicado: regreso desde AZUL.");
      }
...
void finalizarCicloVerdeSinMotor() {
  ...
  Serial.println("========================================");
  Serial.println("CICLO TERMINADO");
  Serial.println("Pieza verde tirada sin mover motor");
  Serial.println("Servo en posicion inicial");
  Serial.println("Sistema listo para otra pieza");
  Serial.println("========================================");
}
...
void controlarMotor() {
  ...
  long errorEnDireccion =
    error * direccionMovimiento;

  if (errorEnDireccion <= 0) {
    ...
    Serial.print("Destino pasado. Motor detenido sin devolverse. Posicion final: ");
    Serial.println(leerPosicion());
  ...
  if (
    (direccionMovimiento > 0 && salidaPID < 0) ||
    (direccionMovimiento < 0 && salidaPID > 0)
  ) {
  ...
  if (distancia <= ZONA_LENTA) {

    long distanciaMapeada = constrain(
      distancia,
      TOLERANCIA_POSICION,
      ZONA_LENTA
    );
  ...
  moverMotorEnDireccion(
    direccionMovimiento,
    pwmActual
  );
}

void moverMotorEnDireccion(
  int direccion,
  int pwm
) {
...
void frenarMotorCorto() {

  analogWrite(MOTOR_PWM, PWM_MIN);
...
void revisarComandosSerial() {
  ...
  if (comando == "START") {
    ...
    emergenciaSerialActiva = false;

    piezaProcesada = false;
    esperandoAntesMovimiento = false;

    Serial.println("OK START");
    Serial.println("Sistema iniciado desde interfaz.");
  ...
  if (
    ...
    emergenciaSerialActiva = true;

    servoMotor.write(SERVO_REPOSO);

    Serial.println("OK EMERGENCY");
    Serial.println("PARO DE EMERGENCIA ACTIVADO DESDE INTERFAZ.");
  ...
  if (comando == "RESET") {
    ...
    Serial.println("Sistema reiniciado desde interfaz.");
    Serial.println("Regresando automaticamente a cero.");

    if (leerPosicion() != POS_CENTRO) {
  ...
  if (comando == "CLEAR_COUNTERS") {

    Serial.println("OK CLEAR_COUNTERS");
    Serial.println("Contadores reiniciados desde interfaz.");
